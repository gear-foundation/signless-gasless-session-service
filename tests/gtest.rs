...
async fn do_something_works() {
    ...
    let mut service_client = sessions_client::Sessions::new(remoting.clone());
    ...
    assert_eq!(result, "Hello from Sessions!".to_string());
}
...
async fn get_something_works() {
    ...
    let service_client = sessions_client::Sessions::new(remoting.clone());
